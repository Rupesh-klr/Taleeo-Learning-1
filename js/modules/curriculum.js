...
if (typeof curriculum === 'undefined') {
    var curriculum = {
        fetchCurriculum: async function(searchTerm = '') {
            ...
            try {
                // Use the centralized helper for the hierarchical search
                const data = await apiGet(`/admin/courses/search?q=${searchTerm}`);
                
                if (data) {
                    this._renderGrid(data.courses);
                } else {
                    // Fallback to local MODULES if backend fails
                    const local = this._getFallbackData(searchTerm);
                    this._renderGrid(local);
                }
            } catch (e) {
                const local = this._getFallbackData(searchTerm);
                this._renderGrid(local);
        ...
        _getFallbackData: function(term) {
